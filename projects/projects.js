...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCategoryPage(dataArray, mainCategoryName) {
        ...
        dataArray.forEach(sub => {
            ...
            // Initial batch: first 3 images
            const initialImages = sub.images.slice(0, 3);
            const remainingImages = sub.images.slice(3);
            ...
            if (remainingImages.length > 0) {
                ...
                btn.addEventListener('click', () => {
                    remainingImages.forEach((imgSrc, index) => {
                        const card = createCard(imgSrc, index + 3);
                        grid.appendChild(card);
                        // Trigger reveal immediately for new cards
                        setTimeout(() => card.classList.add('reveal-visible'), 50);
                    });
                    btnWrap.remove(); // Hide button after showing all
                });

                btnWrap.appendChild(btn);
                subWrap.appendChild(btnWrap);
            }

            container.appendChild(subWrap);
        });
        
        // Re-init reveals for newly added elements
        initScrollReveals();
    }
